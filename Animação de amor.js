...
// Curva de animação personalizada no formato de coração
const easingHeart = mojs.easing.path(
  "M0,100C2.9,86.7,33.6-7.3,46-7.3s15.2,22.7,26,22.7S89,0,100,0"
);
// Elementos que serão usados nas animações
const el = {
  container: qs(".mo-container"),
  // Letras da frase "I LOVE YOU"
  i: qs(".lttr--I"),
  l: qs(".lttr--L"),
  o: qs(".lttr--O"),
  v: qs(".lttr--V"),
...
// Criação de forma personalizada (um coração) para ser usada na animação
class Heart extends mojs.CustomShape {
  getShape() {
    return '<path d="M50,88.9C25.5,78.2,0.5,54.4,3.8,31.1S41.3,1.8,50,29.9c8.7-28.2,42.8-22.2,46.2,1.2S74.5,78.2,50,88.9z"/>'; // Caminho SVG do coração
  }
  getLength() {
    return 200; // Tamanho do caminho
  }
}
// Adiciona o novo formato de coração ao mojs
mojs.addShape("heart", Heart);
// Função que cria o efeito de "explosão" com círculos e partículas
const crtBoom = (delay = 0, x = 0, rd = 46) => {
  ...
  // Partículas que se dispersam (burst)
...
// Função que cria a timeline principal da animação
const crtLoveTl = () => {
  const move = 1000; // Duração dos movimentos principais
  const boom = 200; // Duração dos "explosões"
  ...
  return new mojs.Timeline().add([
    // Reinicializa as letras para visíveis e reseta estilo
    new mojs.Tween({
      duration: move,
      onStart: () => {
    ...
    // Reexibe todas as letras após tudo sumir
    new mojs.Tween({
      duration: 50,
      delay: 4050,
      onUpdate: (progress) => {
    ...
    // Animação da linha da direita (espelhada)
    new mojs.Html({
      ...opts,
      el: el.lineRight,
      x: { 0: -52 },
